    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-card rounded-lg border border-border p-5 shadow-sm">
          ...
          <p className="text-2xl font-bold text-success mt-1">{formatNumber(totalCollected)} د.ل</p>
        </div>
        <div className="bg-card rounded-lg border border-border p-5 shadow-sm">
          <p className="text-sm text-muted-foreground">قيد الانتظار</p>
          <p className="text-2xl font-bold text-warning mt-1">{formatNumber(totalPending)} د.ل</p>
        </div>
      </div>
            ...
            <tbody>
              {transactions.map((t) => (
                <tr key={t.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                  ...
                  <td className="p-3 font-semibold">{formatNumber(t.amount)}</td>
